...
//    uint8_t pressure_it = 1;
    ...
    ui->damuzhi->setRange(600, 1200);
    ui->damuzhi->setValue(800);
    ui->sizhi->setRange(1000, 1160);
...
void MainWindow::setcanid(){
    uint8_t data[16];
    uint16_t pressureBoardId = static_cast<uint16_t>(ui->spinBox_2->value());
    uint16_t ctrlBoardId = static_cast<uint16_t>(ui->spinBox->value());
//    uint16_t pressureId = 65535 - boardId;
    if(comflag){
        data[0] = 0xAA;
        data[1] = 0x00;
